...
import { Button, HStack, IconButton, Stack, Table, Text } from '@chakra-ui/react';
  ...
  if (data.length <= 0) {
    ...
    return (
      <Stack gap={'20px'}>
        <Table.Root>
          <Table.Header>
            <Table.Row bgColor={'rgba(247, 248, 248, 1)'} borderBottom={'2px solid rgba(212, 213, 217, 1)'}>
              {header.map((item, index) => (
                <Table.ColumnHeader p={'4px 12px'} key={index} color={'rgba(102, 106, 121, 1)'}>
                  {item}
                </Table.ColumnHeader>
              ))}
            ...
            {type === 'onHands' &&
              (data as IOnHandsTableData[]).map((item) => (
                <Table.Row key={item.id} p={'12px'} height={'fit-content'}>
                  <Table.Cell>
                    {item.cover}
                    <Text fontFamily={'Inter'} fontWeight={600} fontSize={'16px'} lineHeight={'150%'}>
                      {item.name}
                    </Text>
                  </Table.Cell>
                  ...
                  <Table.Cell display={'flex'} justifyContent={'center'}>
                    <IconButton bgColor={'rgba(0, 0, 0, 0.06)'} rounded={'6px'} w={'45px'} h={'33px'}>
                      <OptionsIcon />
                    </IconButton>
                  </Table.Cell>
                </Table.Row>
            ...
            {(type === 'liked' || type === 'returned') &&
              (data as ILikedReturnedTableData[]).map((item) => (
                <Table.Row key={item.id} p={'12px'}>
                  ...
                  <Table.Cell>{item.dateOfIssuance !== '' ? item.dateOfIssuance : '-'}</Table.Cell>
                  <Table.Cell>{item.dateReturned !== '' ? item.dateReturned : '-'}</Table.Cell>
                  <Table.Cell display={'flex'} justifyContent={'center'}>
                    <IconButton bgColor={'rgba(0, 0, 0, 0.06)'} rounded={'6px'} w={'45px'} h={'33px'}>
                      <OptionsIcon />
                    </IconButton>
        ...
        <HStack hidden={data.length < 12} w={'100%'} justifyContent={'space-between'}>
